<template>
  <div class="time-wordcloud">
    <p class="time-wordcloud__title">金刚经云词条</p>
...
import { getBuddhismFenciApi } from "@/api/common";
...
const option = {
  ...
  tooltip: {
    ...
    formatter: (params) => {
      ...
      <p style="font-size: 16px;color: #6D6A63;line-height: 22px;">数量：${data.value}</p>
    </div>`;
    },
  },
  series: [
...
const getData = async () => {
  const res: any = await getBuddhismFenciApi();
  if (res.code === 0) {
    const data = res.data || [];
